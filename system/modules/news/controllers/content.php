	...
	*/
	public function index()
	{
        ...
        if($records)
        {
	        ...
	        {
	            $image[$record->id] = $this->news_gallery->find_all_by('news_id', $record->id);
	        }
		...
		$total_news = $this->news_model->count_all();
	...
	*/
	private function save_news($type='insert', $id=0)
	{
		...
		if ($type == 'insert')
		{
			...
			if (is_numeric($id) && $this->input->post('images') != '')
			{
            ...
            elseif(is_numeric($id) && $this->input->post('images') == '')
            {
                $return = $id;
            } 
            else
